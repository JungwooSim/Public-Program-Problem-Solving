package section1.ten_DynamicProgramming.four;
...
public class Main {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            brickList.add(
                    new Brick(area, height, weight)
            );
        }

        main.solution(brickList);
    }

    private void solution(ArrayList<Brick> brickList) {
        int answer;
        Collections.sort(brickList);
        int brickSize = brickList.size();
        int[] dy = new int[brickSize];
        ...
        for (int i = 1; i < brickSize; i++) {
            int max_h = 0;
            for (int j = i - 1; j >= 0; j--) {
                if (brickList.get(i).weight < brickList.get(j).weight && dy[j] > max_h) {
                    max_h = dy[j];
                }
            }
